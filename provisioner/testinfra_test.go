...
func TestProvisionerPrepareMinimal(test *testing.T) {
	...
	if len(provisioner.config.LogLevel) > 0 {
		test.Errorf("default empty setting for LogLevel is incorrect: %s", provisioner.config.LogLevel)
	}
